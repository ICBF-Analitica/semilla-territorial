...
### Predicción de Riesgo Nutricional (ST-GNN y S2SLS)
...
El mismo modelo ST-GNN —ya auditado en su capacidad de predicción de riesgo nutricional— se reporta también aplicado a la proyección de tendencia del Z-score de NDVI, alimentando directamente al KPI y al clustering territorial. Según el reporte institucional, la comparación de las predicciones contra los datos reales de un año posterior mostró un error medio absoluto (MAE) de 0.12.

**Recomendación de despliegue del componente predictivo:** dado que el ST-GNN iguala o supera al baseline S2SLS en la tarea de riesgo nutricional (ambos estratos), se recomienda su uso como motor predictivo único para esa tarea específica, manteniendo el S2SLS como referencia de interpretabilidad.
## Limitaciones de Uso

**El componente predictivo de SEMILLA, en su tarea de riesgo nutricional, debe utilizarse exclusivamente como un sistema de alerta temprana de tendencia relativa, no como una herramienta de estimación de casos exacta.** Su función de pérdida está deliberadamente sesgada hacia la sobreestimación en escenarios de alta incertidumbre.
...
1. **Explorar variables predictivas no ambientales en la tarea de riesgo nutricional** El techo de información actual (0.74) sugiere margen de mejora incorporando intervención humanitaria, choques económicos, o migración.
...
Las cifras del componente predictivo en la tarea de riesgo nutricional están respaldadas por evidencia reproducible en docs/auditorias/ (índice completo en docs/auditorias/README.md). Los valores del KPI y del clustering están verificados directamente sobre Avanzado_ia/data/municipios_semilla.csv.
